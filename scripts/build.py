...
def download(url: str, destination: Path) -> None:
    ...
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "homeproxy-rules-builder/1.0"},
    )
...
def collect_rule(rule: dict[str, Any], values: dict[str, set[str]], source: str) -> None:
...
def collect_ruleset(path: Path, values: dict[str, set[str]], source: str) -> int:
...
def normalize_hostname(value: str) -> str:
    ...
    if "://" in value or "/" in value or ":" in value:
        raise BuildError(f"Use a hostname without protocol, port or path: {value!r}")
...
def read_ip_file(path: Path) -> set[str]:
    ...
    for line_number, raw_line in enumerate(
        ...
        try:
            ...
            raise BuildError(f"{path}:{line_number}: invalid IP/CIDR {line!r}") from error
...
def merge_values(target: dict[str, set[str]], source: dict[str, set[str]]) -> None:
    for field, items in source.items():
        target[field].update(items)


def make_default_rule(values: dict[str, set[str]]) -> dict[str, Any]:
    rule: dict[str, Any] = {}
    for field in ADDRESS_FIELDS:
...
def build_source(
    ...
    base_rule = make_default_rule(include_values)
    if not base_rule:
        raise BuildError("The resulting include list is empty")

    exclude_rule = make_default_rule(exclude_values)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(dir=args.output.parent) as temporary_dir:
        ...
        json.loads(verify_json.read_text(encoding="utf-8"))
